...
const permitSchema = z.object({
  ...
  preWorkMeasuresSignature: z.string().optional(),
  preWorkMeasuresDate: z.string().optional(),
  workRemovalSignature: z.string().optional(),
  workRemovalDate: z.string().optional(),
...
export function EditPermitModalUnified({ permit, open, onOpenChange, mode = 'edit', mapClickPosition, onMapReset }: EditPermitModalUnifiedProps) {
  ...
  const form = useForm<PermitFormData>({
    ...
    defaultValues: {
      ...
      status: "draft",
      performerSignature: "",
      preWorkMeasuresSignature: "",
      preWorkMeasuresDate: "",
      workRemovalSignature: "",
      workRemovalDate: "",
      workStartedAt: "",
      workCompletedAt: "",
      additionalComments: "",
  ...
  const submitMutation = useMutation({
    mutationFn: async (data: PermitFormData & { mapPosition?: string }) => {
      ...
      const submitData = {
        ...
        preWorkMeasuresSignature: data.preWorkMeasuresSignature || "",
        preWorkMeasuresDate: data.preWorkMeasuresDate || null,
        workRemovalSignature: data.workRemovalSignature || "",
        workRemovalDate: data.workRemovalDate || null,
  ...
  React.useEffect(() => {
    if (mode === 'edit' && currentPermit && open && !workflowMutation.isPending) {
      ...
      const formData = {
        ...
        preWorkMeasuresSignature: currentPermit.preWorkMeasuresSignature || "",
        preWorkMeasuresDate: formatDate(currentPermit.preWorkMeasuresDate),
        workRemovalSignature: currentPermit.workRemovalSignature || "",
        workRemovalDate: formatDate(currentPermit.workRemovalDate),
